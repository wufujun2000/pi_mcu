...
class CameraService:
    ...
    def camera_connect(self):
        ...
        if self.__camera_brand == "HIKVISION":
            ...
            # )
            self.cap = cv2.VideoCapture(
                # "rtspsrc location=rtsp://%s:%s@%s/Streaming/Channels/%d?transportmode=unicast latency=200 ! queue ! rtph264depay ! h264parse ! v4l2h264dec ! appsink" % (
                "rtspsrc location=rtsp://%s:%s@%s/Streaming/Channels/%d?transportmode=unicast ! rtph264depay ! avdec_h264 ! videoconvert ! videorate ! appsink" % (
                    self.__name, self.__pwd, self.__ip, self.__channel
                ), cv2.CAP_GSTREAMER
            )
    ...
    def get_frame(self):
        ...
        return False if self.__frame_queue.empty() else self.__frame_queue.get()
